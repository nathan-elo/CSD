...
from MCA_modified_test import compute_MC_and_grad_MC
from MCA_modified_test import compute_MC_and_grad_MC_return_intermediate
from MCA_modified_test import compute_y_distrib
from MCA_modified_test import compute_z_rs_1
from MCA_modified_test import compute_z_rs_0
...
def main():
    ...
    """
    origins = [f"O{i}" for i in range(1,3)]
    r_nodes = [f"R{i}" for i in range(1,4)]
    s_nodes = [f"S{i}" for i in range(1,4)]
    destinations = [f"D{i}" for i in range(1,3)]
    ...
    fista = FISTA(subalgorithm, L0=1, eta=5, epsilon=1e-3, max_iter=100)
